...
"We need you to tell us how long Hans' beard was for a handful of months where we didn't measure it."
You being the fortune teller, the ringmaster figures this would be in your wheelhouse.
"Investigators are claiming we lied on some of our advertisements about the length of Hans' amazing beard.
But I'm sure you'll be able to show them that we were telling the truth."

The idea is for you to fill in the blanks for the months where Hans' beard was not measured.
This way the investigators will see that the adverts were true.
To accomplish this task, you are given the historical measurement data.
The independent variable is the amount of months since they started recording the length and the
dependent variable is how long Hans' beard was at the time of measurement.
...
As you study the dots, a string from your robe falls and lays ever so slightly across the dots.

![](/images/2017-01-15-linear-regression-and-the-amazing-beard/data_plot_string_swivel.gif){.post-img .post-img-small .post-img-limit}
...
We will use PureScript to program our simple linear regression calculator.
...
The point-slope form of the equation for a straight line is our hypothesis function.
Our hunch is that the equation of a straight line will best model the data we were given.
...
Just then the ringleader steps into your tent, "Just so you know, if you're wrong, they'll shut us down."
"Oh great," you think to yourself, "If I don't pick the best line, it will cost me my job."
...
So you have a way to make a straight line and you have a way to judge the cost of this line but the question still remains---which line?
...
the greatest possible straight line for modeling our data emerges.
In other words, we minimized the cost function as best we could by repeatedly using the gradient to update our
original guess for the y-intercept and slope.
...
These will be the final learned parameters.
...
If we didn't, this function would continuously call itself.
Note that the recursive call is at the _tail_. This way we won't reach the max stack depth.

Once you have the final y-intercept and slope along with your hypothesis function, you can do the following:

* predict y values using an x value _inside_  the range of x values used to build the model (interpolation)
* predict y values using an x value _outside_ the range of x values used to build the model (extrapolation)

<blockquote>
Extrapolation means creating a tangent line at the end of the known data and extending it beyond that limit.
Linear extrapolation will only provide good results when used to extend the graph of an approximately linear
function or not too far beyond the known data.
<footer>[Extrapolation - Wikipedia](https://en.wikipedia.org/wiki/Extrapolation)</footer>
</blockquote>
...
As you make your way to the next venue, you rush over to the ringmaster.
"I have the missing values," you say with excitement,
"For month two it was 0.056 + 2.755 * 2 = 5.56. For month five it wa...."
However, before you could get another word out, the fire breather interrupts,
"Sir! This fortune teller mistakenly used gradient descent but I took the analytical approach and have a closed form solution."
"I'm not sure what that means," replies the ringmaster, "but it sounds impressive! Let's see what you came up with."

## Wrap-up

From the ground up, we implemented gradient descent and simple linear regression to interpolate the missing values in a given data set.
Along the way, we looked at how each step is achieved in the [interactive demo](http://lettier.com/linear-regression/)
